...
После того, как выполнится стррочка, содержащая объявление переменной, она появится в нашей программе и будет содержать значение, которое мы ей присвоили, пока мы не изменим его.
...
print(triple_and_return(5)) # -> print(15)
# >>> 15
```

Когда в нашем коде есть вызов функции, которая возвращает что-то, то значение, которое она возвращает, после ее вызова подставится вместо нее. То есть, в нашем примере, мы вызываем функцию `triple_and_return()` с параметром `5`, она умножает это число на три и возвращает нам `15`. Это число и подставляется вместо `triple_and_return(5)`. Получается `print(15)`.
...
Квадратный корень какого-нибудь числа (обозначается $\sqrt{\hspace{1em}}$) равен такому числу, которое, при умножении само на себя (то есть, при возведении в квадрат), равно исходному числу. То есть, для числа 9 квадратный корень будет равен числу 3, потому что 3 * 3 = 9. Аналогично, для числа 25 квадратный корень равен 5, потому что 5 * 5 = 25. 

Чтобы найти квадратный корень числа в Python, можно воспользоваться функцией `sqrt()`. Но есть нюанс: чтобы она работала, ее нужно **подключить** (или импортировать), добавив в начало программы строку `from math import sqrt`. Например:
...
### `randint(<число А>, <число B>)`

Чтобы случайное число в Python, можно воспользоваться функцией `randint()`. В скобочках у нее указываются два числа, между которыми (включительно) генерируется случайное. Чтобы эта функция работала, ее нужно **подключить** (или импортировать), добавив в начало программы строку `from random import randint`. Например:

```py
from random import randint

print("Кидаем игральную кость...")
print("Выпало", randint(1, 6))
# >>> Кидаем игральную кость...
# >>> Выпало 4
```